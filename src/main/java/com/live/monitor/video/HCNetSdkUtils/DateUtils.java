...
public class DateUtils {
    ...
    public static Long getDistanceDays(String str1, String str2) throws Exception {
       return getDistanceSeconds(str1,str2)/3600/24;
    }
     ...
     * @return 相差天数
     */
    public static Long getDistanceSeconds(String str1, String str2) throws Exception {
        DateFormat df = new SimpleDateFormat(DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS);
        Date one;
        Date two;
        long seconds = 0;
    ...
    public static Integer getHour(Long timeStamp) {
        ...
        return Integer.parseInt(sdf.format(new Date(timeStamp)));
    }

    /**
     * 获得时间戳的年月日
     * @param timeMills 日期时间戳
     * @return  yyyy-MM-dd格式的日期字符串
     */
    public static String getYMD(long timeMills) {

        return getYMD(new Date(timeMills));
    }
    /**
     * 获得时间戳的年月日
     * @param date 日期
     ...
     * @return
     */
    public static boolean isInTheDistance(Date comeTime, Date start, Date end) {
        if (comeTime.after(start)&&comeTime.before(end)){
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    public static Date lastTime(Date beginTime) {
        Calendar c = Calendar.getInstance();
        c.setTime(beginTime);
        //c.add(Calendar.SECOND, +rebootTime); // 控制秒
        c.add(Calendar.MINUTE, + 1); //控制分
        //c.set(Calendar.HOUR_OF_DAY, +rebootTime);//控制时
        lastTime = c.getTime();
        return lastTime;
    }
    /**
     ...
     * @return
     */
    public static Date lastTimeAddDay (Date beginTime) {
        Calendar c = Calendar.getInstance();
        c.setTime(beginTime);
        //c.add(Calendar.SECOND, +rebootTime); // 控制秒
        //c.add(Calendar.MINUTE, + 30); //控制分
        c.add(Calendar.HOUR_OF_DAY, + 13);//控制时
        lastTime = c.getTime();
        return lastTime;
    }
}
